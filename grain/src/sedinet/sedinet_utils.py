...
def get_data_generator_Nvars_siso_simo(
    df, indices, for_training, vars, batch_size, greyscale, CS, do_aug
):  ##BATCH_SIZE
    """
            ...
            if greyscale == True:
                if do_aug == True:
                    ...
                    if len(vars) == 1:
                        ...
                        p1s.append([p1 for k in range(2)])
                        p2s.append([p2 for k in range(2)])
                    elif len(vars) == 3:
                        p1s.append([p1 for k in range(2)])
                        p2s.append([p2 for k in range(2)])
                        ...
                        p2s.append([p2 for k in range(2)])
                        p3s.append([p3 for k in range(2)])
                        p4s.append([p4 for k in range(2)])
                    elif len(vars) == 5:
                        p1s.append([p1 for k in range(2)])
                        p2s.append([p2 for k in range(2)])
                        p3s.append([p3 for k in range(2)])
                        ...
                        p4s.append([p4 for k in range(2)])
                        p5s.append([p5 for k in range(2)])
                        p6s.append([p6 for k in range(2)])
                    elif len(vars) == 7:
                        p1s.append([p1 for k in range(2)])
                        p2s.append([p2 for k in range(2)])
                        p3s.append([p3 for k in range(2)])
                        ...
                        p6s.append([p6 for k in range(2)])
                        p7s.append([p7 for k in range(2)])
                        p8s.append([p8 for k in range(2)])
                    elif len(vars) == 9:
                        p1s.append([p1 for k in range(2)])
                        p2s.append([p2 for k in range(2)])
                        p3s.append([p3 for k in range(2)])
                        ...
                        if len(images) >= batch_size:
                            if greyscale == False:
                                ...
                                images = np.expand_dims(
                                    np.array(np.vstack(images)), axis=-1
                                )
                        ...
                        if do_aug == True:
                            if len(images) >= batch_size:
                                if greyscale == False:
                                    ...
                                    p1s = np.expand_dims(
                                        np.vstack(p1s).flatten(), axis=-1
                                    )
                                    p2s = np.expand_dims(
                                        np.vstack(p2s).flatten(), axis=-1
                                    )
                                    p3s = np.expand_dims(
                                        np.vstack(p3s).flatten(), axis=-1
                                    )
                                    p4s = np.expand_dims(
                                        np.vstack(p4s).flatten(), axis=-1
                                    )
                                    p5s = np.expand_dims(
                                        np.vstack(p5s).flatten(), axis=-1
                                    )
                                    p6s = np.expand_dims(
                                        np.vstack(p6s).flatten(), axis=-1
                                    )
                                    p7s = np.expand_dims(
                                        np.vstack(p7s).flatten(), axis=-1
                                    )
                                    p8s = np.expand_dims(
                                        np.vstack(p8s).flatten(), axis=-1
                                    )
                                    p9s = np.expand_dims(
                                        np.vstack(p9s).flatten(), axis=-1
                                    )
                                yield images, (
                                    p1s,
                                    p2s,
...
###===================================================
def get_data_generator_1image(
    df, indices, for_training, ID_MAP, var, batch_size, greyscale, do_aug
):
    """
        ...
        for i in indices:
            ...
            if greyscale == True:
                if do_aug == True:
                    ...
                    images.append(aug)
                    pops.append(
                        [to_categorical(pop, len(ID_MAP2)) for k in range(2)]
                    )  # 3
                else:
            ...
            try:
                ...
                if do_aug == True:
                    ...
                    if len(images) >= batch_size:
                        yield np.squeeze(np.array(images)), np.array(
                            pops
                        )  # [np.array(pops)]
                        images, pops = [], []
...
def plot_train_history_Nvar(history, varuse, N):
    ...
    for k in range(N):
        try:
            axes[k].plot(
                history.history[varuse[k] + "_output_mean_absolute_error"],
                label=varuse[k] + " Train MAE",
            )
            axes[k].plot(
                history.history["val_" + varuse[k] + "_output_mean_absolute_error"],
                label=varuse[k] + " Val MAE",
            )
        except:
            axes[k].plot(
                history.history[varuse[k] + "_output_mae"],
                label=varuse[k] + " Train MAE",
            )
            axes[k].plot(
                history.history["val_" + varuse[k] + "_output_mae"],
                label=varuse[k] + " Val MAE",
            )
...
def plot_confusion_matrix(
    cm, classes, normalize=False, cmap=plt.cm.Purples, dolabels=True
):
    """
...
def predict_test_train_cat(
    ...
    train_gen = get_data_generator_1image(
        train_df,
        train_idx,
        False,
        len(classes),
        var,
        np.min((1000, len(train_idx))),
        greyscale,
        do_aug,
    )
    ...
    if type(SM) == list:
        ...
        for s in SM:
            ...
            exec("PT.append(np.asarray(np.squeeze(tmp)))")
            del tmp
    ...
    if test_df is not None:
        ...
        test_gen = get_data_generator_1image(
            test_df,
            test_idx,
            False,
            len(classes),
            var,
            np.min((1000, len(test_idx))),
            greyscale,
            False,
    ...
    if type(SM) == list:
        ...
        if test_df is not None:
            plot_confmat(pred, true, var, classes)
            plt.savefig(
                weights_path.replace(".weights.h5", "_cm.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close("all")
        ...
        plot_confmat(predT, trueT, var + "T", classes)
        plt.savefig(
            weights_path.replace(".weights.h5", "_cmT.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close("all")
...
###===================================================
def predict_test_train_siso_simo(
    train_df,
    test_df,
    train_idx,
    test_idx,
    ...
    train_gen = get_data_generator_Nvars_siso_simo(
        train_df,
        train_idx,
        False,
        vars,
        np.min((400, len(train_idx))),
        greyscale,
        CS,
        False,
    )  #
    ...
    if scale == True:
        ...
        if type(SM) == list:
            ...
            if len(vars) > 1:
                for v in vars:
                    exec(v + "_PT = np.median(" + v + "_PT, axis=0)")
            else:
                exec(vars[0] + "_PT = np.median(" + v + "_PT, axis=0)")
        ...
        if type(SM) == list:
            for s in SM:
                ...
                if len(vars) > 1:
                    counter2 = 0
                    for v in vars:
                        exec(v + "_PT.append(np.squeeze(tmp[counter2]))")
                        counter2 += 1
                else:
                    exec(vars[0] + "_PT.append(np.asarray(tmp))")
    ...
    ## make predictions on testing data
    if test_df is not None:
        test_gen = get_data_generator_Nvars_siso_simo(
            test_df,
            test_idx,
            False,
            vars,
            np.min((400, len(test_idx))),
            greyscale,
            CS,
            False,
        )  #
        ...
        if scale == True:
            ...
            if type(SM) == list:
                ...
                if len(vars) > 1:
                    for v in vars:
                        exec(v + "_P = np.median(" + v + "_P, axis=0)")
                else:
                    exec(vars[0] + "_P = np.median(" + v + "_P, axis=0)")
    ...
    for k in range(1, 1 + (nrows * ncols)):
        try:
            ...
            if test_df is not None:
                plt.text(
                    np.nanmin(x2),
                    0.75 * np.max(np.hstack((x2, y2))),
                    "Test : " + str(np.mean(100 * (np.abs(y2 - x2) / x2)))[:5] + " %",
                    fontsize=10,
                    color="b",
                )
                plt.text(
                    np.nanmin(x1),
                    0.7 * np.max(np.hstack((x1, y1))),
                    "Train : " + str(np.mean(100 * (np.abs(y1 - x1) / x1)))[:5] + " %",
                    fontsize=10,
                )
            else:
                plt.text(
    ...
    if type(SM) == list:
        plt.savefig(
            weights_path[0]
            .replace(".weights.h5", "_skill_ensemble.png")
            .replace("batch", "_".join(np.asarray(BATCH_SIZE, dtype="str"))),
            dpi=300,
            bbox_inches="tight",
        )
        joblib.dump(
            Z,
            weights_path[0]
            .replace(".weights.h5", "_bias.pkl")
            .replace("batch", "_".join(np.asarray(BATCH_SIZE, dtype="str"))),
        )

    else:
        plt.savefig(
            weights_path.replace(".weights.h5", "_skill.png"),
            dpi=300,
            bbox_inches="tight",
        )
        joblib.dump(Z, weights_path.replace(".weights.h5", "_bias.pkl"))
